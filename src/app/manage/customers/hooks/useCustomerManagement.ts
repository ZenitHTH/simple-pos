...
export function useCustomerManagement() {
  ...
  const handleModalSubmit = async (
    ...
    try {
      ...
      let result: Customer;
      if (editingCustomer) {
        result = await customerApi.update(dbKey, editingCustomer.id, data);
        updateCache.customers(
          customers.map((c) => (c.id === result.id ? result : c)),
        );
      } else {
  ...
  const filteredCustomers = customers.filter((customer) => {
    return (
      ...
      (customer.tax_id &&
        customer.tax_id.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (customer.address &&
