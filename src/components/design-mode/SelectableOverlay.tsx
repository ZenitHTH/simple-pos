  ...
  const getScaleValue = (id: string): number => {
    switch (id) {
      ...
      case "manage_table_scale":
        return settings.scaling.components.manage_table;
      case "stock_table_scale":
        return settings.scaling.components.stock_table;
      case "material_table_scale":
        return settings.scaling.components.material_table;
      case "category_table_scale":
        return settings.scaling.components.category_table;
      case "setting_page_scale":
        return settings.scaling.components.setting_page;
      case "payment_modal_scale":
        return settings.scaling.components.payment_modal;
      default:
        return 100;
    }
  };
  ...
  const updateScaleValue = (id: string, val: number) => {
    switch (id) {
      ...
      case "manage_table_scale":
        ...
        break;
      case "stock_table_scale":
        updateSettings({ scaling: { components: { stock_table: val } } });
        break;
      case "material_table_scale":
        updateSettings({ scaling: { components: { material_table: val } } });
        break;
      case "category_table_scale":
  ...
  const renderHandle = (direction: "nw" | "ne" | "sw" | "se") => {
    ...
    return (
      <motion.div
        ...
        className={cn(
          "bg-primary border-background absolute z-50 flex h-6 w-6 items-center justify-center rounded-full border-4 shadow-xl transition-all",
    ...
    <div
      ...
      className={cn(
        ...
        isSelected
          ...
          : "hover:border-primary/50 hover:bg-primary/5 border-2 border-transparent",
      ...
      {isSelected && (
        <>
          ...
          <motion.div
            ...
            className="border-muted-foreground/30 pointer-events-none absolute inset-0 -z-1 border-2 border-dashed opacity-50"
            style={{
